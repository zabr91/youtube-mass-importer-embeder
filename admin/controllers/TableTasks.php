...
class TableTasks extends WP_List_Table {
	...
	function column_default( $item, $colname ){
		...
		if( $colname === 'actions' ){
			...
			$actions = array();
			$actions['edit'] = sprintf( '<a href="%s">%s</a>', '?page='.$_GET['page'].'&action=edit&id='.$item->id,'edit' );
			$actions['delete'] = sprintf( '<a href="%s">%s</a>', '?page='.$_GET['page'].'&action=deleteprice&id='.$item->id, 'delete' );
